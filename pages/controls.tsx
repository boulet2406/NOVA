...
import { FileText, ChartPie, TrendingUp, TrendingDown, ChevronDown } from 'lucide-react'
...
const allCampaigns: CampaignData[] = [
  { name: 'Campagne A – 2025', appliedFormalized: 80, partialFormalized: 10, notApplicable: 5, failing: 3, empty: 1, none: 1 },
  { name: 'Campagne B – 2025', appliedFormalized: 70, partialFormalized: 20, notApplicable: 2, failing: 5, empty: 2, none: 1 },
  { name: 'Campagne C – 2024', appliedFormalized: 85, partialFormalized:  8, notApplicable: 4, failing: 2, empty: 1, none: 0 },
  { name: 'Campagne D – 2023', appliedFormalized: 90, partialFormalized:  5, notApplicable: 3, failing: 1, empty: 1, none: 0 },
...
export default function ControlsPage() {
  ...
  const lineData = useMemo(() => {
    return years.map(y => {
      ...
      const avg = items.reduce((sum, c) => sum + c.appliedFormalized, 0) / items.length
  ...
  const flat = data.flatMap(c =>
    Object.entries(COLORS).map(([key]) => (c as any)[key] as number)
  )
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-900 text-gray-900 dark:text-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[
            { title: 'Moyenne', value: `${avgKPI}%`, icon: <TrendingUp className="text-green-500"/> },
            { title: 'Minimum', value: `${mn}%`, icon: <TrendingDown className="text-orange-500"/> },
            { title: 'Maximum', value: `${mx}%`, icon: <TrendingUp className="text-blue-500"/> },
          ].map(kpi => (
            <div
              key={kpi.title}
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <FileText className="text-indigo-500"/> Auto-évaluation {year}
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* 1) BarChart */}
          <div className="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-lg overflow-auto" style={{ maxHeight: 400 }}>
            <ResponsiveContainer width="100%" height={data.length * 60}>
              <BarChart
                layout="vertical"
                data={data}
                margin={{ top: 8, right: 20, left: 180, bottom: 8 }}
                barCategoryGap="20%"
              >
          ...
          <div className="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-lg">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <ChartPie className="text-purple-500"/> Répartition globale
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                ...
                >
                  {pieData.map((_, idx) => {
                    const color = (Object.values(COLORS) as string[])[idx]
          ...
          <div className="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-lg">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <TrendingUp className="text-blue-500"/> Tendance Moyenne
            </h3>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={lineData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                ...
                <YAxis domain={[0,100]} />
                <Tooltip formatter={(v:number)=>`${v}%`} />
                <Line type="monotone" dataKey="avg" stroke="#2563EB" strokeWidth={3} dot={{ r:4 }} activeDot={{ r:6 }}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-lg">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <TrendingDown className="text-yellow-500"/> Échecs par année
            </h3>
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={areaData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                <defs>
                ...
                <Tooltip formatter={(v:number)=>v}/>
                <Area type="monotone" dataKey="sum" stroke="#f59e0b" fill="url(#grad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
