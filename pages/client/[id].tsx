...
useEffect(() => {
  ...
  const saveComment = (preset?: string, newStatus?: string) => {
    ...
    const list = [{ date, time, user: currentMatricule, text: txt }, ...(comments[client.id]||[])].slice(0,5);
  ...
  const navigate = (dir: -1|1) => {
    ...
    if (ni >= 0 && ni < clients.length) {
      router.push(`/client/${clients[ni].id}`);
    }
  };

const exportPDF = () => {
  const pdf = generateClientReport(client, comments, statuses, auditTrail);
    ...
    <div className="min-h-screen bg-zinc-950 text-white font-sans overflow-auto">
      <Toaster position="bottom-right" />
        ...
        <div className="space-y-8">
          <Card>
            <div className="flex items-center gap-1">
              <InfoIcon className="h-5 w-5 text-zinc-400" role="img" aria-label="Info" />
          ...
          <Accordion
            title={
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-green-400">{client.riskScore}/100</span>
              </div>
            }
            defaultOpen={true}
          >
          ...
          <Accordion
            ...
            title={
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <InfoIcon className="h-5 w-5 text-cyan-400" role="img" aria-label="Informations" />
                  <span className="text-sm font-semibold uppercase text-zinc-400">
                    Informations
                  </span>
                  <HelpCircle
          ...
          <div ref={chartRef}>
            <Card>
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-cyan-400">{opScore}/100</span>
              </div>
              <div className="h-40 bg-zinc-800 p-2 rounded">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={client.scoreHistory}>
                    ...
                    <Tooltip />
                    <Line dataKey="score" stroke="#38bdf8" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
        ...
        <Accordion
          ...
          title={
            <div className="flex items-center gap-2">
              <Bell className="h-5 w-5 text-yellow-400" role="img" aria-label="Alertes comportementales" />
              <span className="text-sm font-semibold uppercase text-zinc-400">
                Scoring Alerting
              </span>
              <HelpCircle
        ...
        >
          <List items={[
            { label: 'Jeux à risque',       value: client.indicateursComportementaux.jeuxARisque },
            { label: 'Vitesse',             value: client.indicateursComportementaux.vitesseJeu },
            { label: 'Dernière IP',         value: client.indicateursComportementaux.dernierChangementIP },
            { label: 'Device',              value: client.indicateursComportementaux.deviceInhabituel },
            { label: 'Tiers payant',        value: client.indicateursComportementaux.tiersPayant ? 'Oui' : 'Non' },
          ]} />
        </Accordion>
        ...
        <div className="space-y-8 relative">
          <Card>
            <div className="flex items-center gap-2">
              <Bell className="h-5 w-5 text-red-400" role="img" aria-label="Alertes" />
              <h2 className="text-sm font-semibold uppercase text-zinc-400">
                Historique Alertes
                <HelpCircle
                  className="inline-block h-4 w-4 ml-1 text-zinc-500 hover:text-zinc-300"
          ...
          >
            <ChevronUp className="h-5 w-5 text-white" role="img" aria-label="Haut" />
          </button>
        </div>
      </main>
    </div>
